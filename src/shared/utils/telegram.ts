 ...
 * - Mini App deep links
 *
 * IMPORTANT: Referral codes are currently generated client-side for demo purposes.
 * In production, referral codes MUST be generated by the backend server:
 * - Server generates unique secure tokens (e.g., random base64url strings)
 * - Tokens are stored in database with user_id mapping
 * - No personal information (username, id) should be exposed in referral codes
 * - Backend validates incoming referral codes and tracks referrals
 *
...
export const buildReferralCode = (profile: TelegramProfile | null): string | null => {
  ...
  // Генерируем безопасный реферальный код на основе user ID
  // В production это должен делать backend, возвращая уникальный токен
  // Временно используем base64url кодирование ID для демонстрации
  const base64Id = btoa(`user_${profile.id}`)
    .replace(/\+/g, "-")
    .replace(/\//g, "_")
    .replace(/=/g, "");

  return base64Id.slice(0, 16); // Ограничиваем длину для компактности
};
...
/**
 * Builds referral links for sharing
 *
 * @param code - Referral code (in production, should come from backend API)
 * @param options - Additional options like campaign tracking
 * @returns Object containing different link formats (universal, webApp, bot, native)
 *
 * Link formats:
 * - universal: Best for sharing, auto-detects platform
 * - webApp: Direct Mini App link (uses startapp parameter)
 * - bot: Opens bot chat (uses start parameter)
 * - native: tg:// protocol for deep linking
 *
 * Example usage:
 * const links = buildReferralLinks('abc123xyz');
 * // links.universal = https://t.me/YourBot?startapp=ref__abc123xyz
 */
